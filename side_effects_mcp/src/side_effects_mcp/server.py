...
@app.tool
def get_side_effects_for_drugs(drug_name: Annotated[str, "The name of the medical drug to find side effects for"]) -> list[dict]:
    """Get side effects for a given drug name from clinicaltrials.gov"""
    ...
    return [x for x in results if x['side_effect_probability'] > 0]
